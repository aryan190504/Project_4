...
public class CollegeModel {
	...
	public long add(CollegeBean bean) throws ApplicationException,DuplicateRecordException{
		...
		try{
			...
			PreparedStatement pstmt=conn.prepareStatement("INSERT INTOst_collage VALUES(?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
	...
	public void delete(CollegeBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn.prepareStatement("DELETE FROMst_collage WHERE ID=?");
	...
	public CollegeBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROMst_collage WHERE NAME=?");
	...
	public CollegeBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT*FROMst_collage WHERE id=?");
		CollegeBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public void update(CollegeBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATEst_collage SET NAME=?,ADDRESS=?,STATE=?,CITY=?,PHONE_NO=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
	...
	public List search(CollegeBean bean, int pageNo, int PageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROMst_collage WHERE 1=1");
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * fromst_collage");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
