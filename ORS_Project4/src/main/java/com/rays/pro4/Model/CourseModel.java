...
//				 throw new ApplicationException("Exception in getting course by name");
		 ...
		 public CourseBean FindByPK(long pk) throws ApplicationException {
			 ...
			 StringBuffer sql=new StringBuffer("SELECT * FROMst_course WHERE ID=?");
			 Connection conn =  null;
			 CourseBean bean = null;
			 try {
				 conn=JDBCDataSource.getConnection();
		 ...
		 public void update(CourseBean bean) throws ApplicationException, DuplicateRecordException {
			 ...
			 try {
				 conn=JDBCDataSource.getConnection();
				 conn.setAutoCommit(false);
				 PreparedStatement pstmt = conn.prepareStatement("UPDATEst_course SET NAME=?,DESCRIPTION=?,DURATION=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
...
//				 throw new ApplicationException("Exception in updatingcourse" );
		 ...
		 public List search(CourseBean bean,int pageNo,int pageSize) throws DatabaseException, ApplicationException {
			 ...
			 StringBuffer sql= new StringBuffer("Select * fromst_course where 1=1");
			 if(bean !=null) {
				 if(bean.getId() > 0) {
		 ...
		 public List list() throws Exception {
		  ...
		  StringBuffer sql = new StringBuffer("select * fromst_course");
